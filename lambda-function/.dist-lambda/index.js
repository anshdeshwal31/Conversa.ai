...
async function syncUserCalendar(user) {
    try {
        ...
        const deletedEvents = existingEvents.filter(dbEvent => !googleEventIds.has(dbEvent.calendarEventId));
...
async function processEvent(user, event) {
    ...
    try {
        ...
        if (exsistingMeeting) {
            const changes = [];
            if (exsistingMeeting.title !== eventData.title)
                changes.push('title');
            if (exsistingMeeting.startTime.getTime() !== eventData.startTime.getTime())
                changes.push('time');
